...
class LabelWeights(BaseModel):
    ...
    @classmethod
    def validate_labels_and_weights(
        cls, label_weights: Dict[str, float]
    ) -> Dict[str, float]:
        ...
        for label, weight in label_weights.items():
            if len(label) > MAX_LABEL_LENGTH:
                bt.logging.warning(
                    f"Skipping label {label}: exceeds {MAX_LABEL_LENGTH} character limit"
                )
                continue
            ...
            if weight <= 0:
                bt.logging.warning(
                    f"Skipping label {label} with non-positive weight: {weight}"
                )
                continue
...
class OldFormatPreference(BaseModel):
    ...
    def validate_source(cls, source_name: str) -> str:
        ...
        if source_lower not in VALID_SOURCES:
            raise ValueError(
                f"Invalid source: {source_name}. Must be one of {VALID_SOURCES}"
            )
        return source_lower
...
class JobParams(BaseModel):
    ...
    platform: str = Field(
        description="Platform to search (e.g., 'reddit', 'x', 'youtube')"
    )
    ...
    post_start_datetime: Optional[str] = Field(
        None, description="Start date for posts in ISO format"
    )
    post_end_datetime: Optional[str] = Field(
        None, description="End date for posts in ISO format"
    )
    ...
    def validate_platform(cls, platform: str) -> str:
        ...
        if platform_lower not in VALID_SOURCES:
            raise ValueError(
                f"Invalid platform: {platform}. Must be one of {VALID_SOURCES}"
            )
    ...
    def validate_datetime_format(cls, date_string: Optional[str]) -> Optional[str]:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid datetime format: {date_string}. Must be ISO format"
            )
    ...
    def validate_job_params(self) -> "JobParams":
        ...
        if self.platform == "youtube":
            if self.keyword is not None and self.label is not None:
                raise ValueError(
                    "YouTube jobs cannot have both keyword and label - use either keyword (video URL) OR label (channel)"
                )
            if self.keyword is None and self.label is None:
                raise ValueError(
                    "YouTube jobs must have either keyword (video URL) OR label (channel)"
                )
...
class PreferencesData(BaseModel):
    ...
    def parse_and_normalize(
        cls, data: List[dict], hotkey: Optional[str] = None
    ) -> List[Job]:
        """
        ...
        if is_new_format:
            ...
            bt.logging.info(
                "Processing old format preferences, will convert to new format"
            )
            if not hotkey:
                bt.logging.error(
                    "Hotkey required for converting old format to new format"
                )
                raise ValueError(
                    "Hotkey required for converting old format to new format"
                )
            ...
            try:
                ...
                for source_dict in data:
                    try:
                        ...
                        if (
                            valid_source.label_weights
                        ):  # Only include if it has valid labels
    ...
    def _convert_to_new_format(
        sources: List[OldFormatPreference], hotkey: str
    ) -> List[Job]:
