...
class JobParams(BaseModel):
    ...
    keyword: str | None = Field(
        None, description="Optional keyword to search for", min_length=1, max_length=MAX_LABEL_LENGTH
    )
    platform: str = Field(description="Platform to search (e.g., 'reddit', 'x', 'youtube')")
    label: str | None = Field(
        None, description="Optional label or label to search for", min_length=1, max_length=MAX_LABEL_LENGTH
    )
    post_start_datetime: str | None = Field(None, description="Start date for posts in ISO format")
    post_end_datetime: str | None = Field(None, description="End date for posts in ISO format")
    ...
    @classmethod
    def validate_datetime_format(cls, date_string: str | None) -> str | None:
...
class Job(BaseModel):
    ...
    id: str = Field(
        max_length=80, description="Unique identifier for the job. Must be 80 characters or less with no slashes."
    )
...
class PreferencesData(BaseModel):
    ...
    @classmethod
    def parse_and_normalize(cls, data: list[dict], hotkey: str | None = None) -> list[Job]:
        """
...
def normalize_preferences(data: list[dict], hotkey: str | None = None) -> str:
    """
