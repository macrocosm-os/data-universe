...
def calculate_total_weights(
    ...
    total_vali_weight: float = TOTAL_VALI_WEIGHT,
) -> None:
    ...
    try:
        ...
        for job in default_jobs:
            if "params" not in job or not all(
                k in job["params"] for k in ["keyword", "platform", "label"]
            ):
        ...
        bt.logging.error(
            f"Warning: {default_json_path} not found. Proceeding without default jobs."
        )

    # Calculate the total stake of validators who have voted
    total_stake = sum(
        v.get("percent_stake", 1) for v in validator_data.values() if v.get("json")
    )
    ...
    for hotkey, data in validator_data.items():
        ...
        if validator_json and isinstance(validator_json, list):
            if (
                "source_name" in validator_json[0]
                and "label_weights" in validator_json[0]
            ):
        ...
        for job in validator_json:
            ...
            if (
                job["params"]["label"]
                and len(job["params"]["label"]) > constants.MAX_LABEL_LENGTH
            ):
                continue
    ...
    for job_key, job_data in aggregated_jobs.items():
        job = {
            "id": job_ids[
                job_key
            ],  # Use the preserved ID instead of generating new one
...
def to_lookup(json_path: str):
    ...
    for data_source in active_data_sources:
        ...
        platform_jobs = [
            job for job in jobs if job["params"].get("platform") == platform_name
        ]
        ...
        job_matcher = JobMatcher(
            jobs=[
                Job(
                    ...
                    keyword=job["params"].get(
                        "keyword"
                    ),  # Use get() with default None to handle missing keys
                    label=job["params"].get("label"),
                    job_weight=job["weight"],
                    # Convert datetime strings to time bucket IDs during initial parsing
                    start_timebucket=datetime_to_timebucket(
                        job["params"].get("post_start_datetime")
                    ),
                    end_timebucket=datetime_to_timebucket(
                        job["params"].get("post_end_datetime")
                    ),
                )
    ...
    return DataDesirabilityLookup(
        distribution=distribution, max_age_in_hours=max_age_in_hours
    )
...
def get_hotkey_json_submission(
    subtensor: bt.subtensor, netuid: int, metagraph: bt.metagraph, hotkey: str
):
    ...
    try:
        if not hotkey:
            bt.logging.info(
                f"No hotkey specified. Returning aggeregate dynamic desirability list."
            )
            script_dir = os.path.dirname(os.path.abspath(__file__))
        ...
        if is_validator(uid=uid, metagraph=metagraph):
            bt.logging.info(
                f"Hotkey {hotkey} is a validator. Checking for JSON submission to return..."
            )
            commit_sha = subtensor.get_commitment(netuid=netuid, uid=uid)
            return get_json(commit_sha=commit_sha, filename=f"{hotkey}.json")
        else:
            bt.logging.error(
                f"Hotkey {hotkey} is not a validator. Only validators have JSON submissions. "
            )
            return None

    except Exception as e:
        bt.logging.error(
            f"Could not retrieve JSON submission for hotkey {hotkey}. Error: {e}"
        )
        return None
...
async def run_retrieval(config) -> DataDesirabilityLookup:
    ...
    try:
        ...
        validator_data = get_validator_data(
            metagraph=metagraph, vpermit_rao_limit=config.vpermit_rao_limit
        )
        ...
        for hotkey in validator_data.keys():
            uid = subtensor.get_uid_for_hotkey_on_subnet(
                hotkey_ss58=hotkey, netuid=config.netuid
            )
            validator_data[hotkey]["github_hash"] = subtensor.get_commitment(
                netuid=config.netuid, uid=uid
            )
            if validator_data[hotkey]["github_hash"]:
                validator_data[hotkey]["json"] = get_json(
                    commit_sha=validator_data[hotkey]["github_hash"],
        ...
        bt.logging.error(
            f"Could not retrieve dynamic preferences. Using default.json to build lookup: {str(e)}"
        )
        script_dir = os.path.dirname(os.path.abspath(__file__))
        return to_lookup(os.path.join(script_dir, DEFAULT_JSON_PATH))


def _dd_list_to_jobs_json(
    dd_list: DynamicDesirabilityList, apply_floor: bool = True
) -> list:
    ...
    for entry in dd_list.entries:
        ...
        post_start = (
            entry.post_start_datetime.isoformat() if entry.post_start_datetime else None
        )
        post_end = (
            entry.post_end_datetime.isoformat() if entry.post_end_datetime else None
        )
...
async def run_retrieval_from_api(
    ...
    try:
        ...
        bt.logging.info(
            f"Received DD list version={dd_list.version} with {len(dd_list.entries)} entries"
        )
        ...
        bt.logging.error(
            f"Failed to fetch DD list from API: {str(e)}. Falling back to default.json"
        )
        script_dir = os.path.dirname(os.path.abspath(__file__))
        return to_lookup(os.path.join(script_dir, DEFAULT_JSON_PATH))
