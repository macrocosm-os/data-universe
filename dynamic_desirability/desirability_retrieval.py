...
from common.utils import get_validator_data, is_validator, parse_iso_date, time_bucket_id_from_datetime
...
def get_json(commit_sha: str, filename: str) -> dict[str, Any] | None:
...
def calculate_total_weights(
    ...
    try:
        ...
        for job in default_jobs:
            ...
            aggregated_jobs[job_key] = {"weight": job.get("weight", 1.0), "params": job_params}
    ...
    for hotkey, data in validator_data.items():
        ...
        for job in validator_json:
            ...
            if job_key in aggregated_jobs:
                ...
                aggregated_jobs[job_key] = {"weight": weighted_job_value, "params": job_params}
...
def to_lookup(json_path: str):
    ...
    for data_source in active_data_sources:
        ...
        distribution[data_source] = DataSourceDesirability(
            weight=data_source.weight, default_scale_factor=DEFAULT_SCALE_FACTOR, job_matcher=job_matcher
        )
...
def datetime_to_timebucket(datetime_str: str | None) -> int | None:
...
def get_hotkey_json_submission(subtensor: bt.subtensor, netuid: int, metagraph: bt.metagraph, hotkey: str):
    ...
    try:
        if not hotkey:
            bt.logging.info("No hotkey specified. Returning aggeregate dynamic desirability list.")
            script_dir = os.path.dirname(os.path.abspath(__file__))
            agg_path = os.path.join(script_dir, AGGREGATE_JSON_PATH)
            with open(agg_path) as file:
...
async def run_retrieval(config) -> DataDesirabilityLookup:
    ...
    try:
        ...
        for hotkey in validator_data.keys():
            ...
            if validator_data[hotkey]["github_hash"]:
                validator_data[hotkey]["json"] = get_json(
                    commit_sha=validator_data[hotkey]["github_hash"], filename=f"{hotkey}.json"
                )
        ...
        calculate_total_weights(
            validator_data=validator_data, default_json_path=default_path, total_vali_weight=TOTAL_VALI_WEIGHT
        )
