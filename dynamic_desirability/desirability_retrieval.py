...
from data_universe_api import DataUniverseApiClient
...
def to_lookup(json_path: str):
    ...
    for data_source in active_data_sources:
        ...
        distribution[data_source] = DataSourceDesirability(
            weight=data_source.weight, job_matcher=job_matcher
        )
...
async def run_retrieval(
    data_universe_api_client: DataUniverseApiClient,
    mode: Literal["miner"] | Literal["validator"],
) -> DataDesirabilityLookup:
    try:
        bt.logging.info(
            "\nGetting off-chain dynamic desirability list from data-universe-api...\n"
        )

        coro = (
            data_universe_api_client.miner_get_latest_dd_list()
            if mode == "miner"
            else data_universe_api_client.validator_get_latest_dd_list()
        )
        ...
        with open(agg_path, "w") as f:
            json.dump([entry.model_dump() for entry in dd_list.entries], f, indent=4)

        bt.logging.info("Saved latest DD list to aggregate.json")
        return to_lookup(agg_path)
    except Exception as e:
        bt.logging.error(
            f"Failed to fetch DD list from API, falling back to default.json: {str(e)}"
        )
        script_dir = os.path.dirname(os.path.abspath(__file__))
...
def sync_run_retrieval(
    ...
    return asyncio.run(run_retrieval(data_universe_api_client, mode))
