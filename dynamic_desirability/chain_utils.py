...
def add_args(parser: argparse.ArgumentParser, is_upload: bool):
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of the subtensor network",
        default="finney",
    )
    parser.add_argument(
        "--netuid", type=int, required=True, help="UID of the subnet", default=13
    )
    ...
    if is_upload:
        parser.add_argument(
            ...
            type=str,
            required=True,
            help="Path to the JSON file containing preferences",
        )
...
class ChainPreferenceStore:
    ...
    async def batch_retrieve_preferences(
        self, hotkeys: list[str], chunk_size: int = 5
