...
class CompressedMinerIndex(BaseModel):
    ...
    def validate_index_size(
        ...
        if size > constants.DATA_ENTITY_BUCKET_COUNT_LIMIT_PER_MINER_INDEX_PROTOCOL_5:
            raise ValueError(
                f"Compressed index is too large. {size} buckets > {constants.DATA_ENTITY_BUCKET_COUNT_LIMIT_PER_MINER_INDEX_PROTOCOL_5}"
            )
        return sources
