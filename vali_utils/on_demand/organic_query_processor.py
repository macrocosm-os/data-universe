...
from vali_utils.on_demand.output_models import (
    create_organic_output_dict,
...
class OrganicQueryProcessor:
    ...
    async def process_organic_query(self, synapse: OrganicRequest) -> OrganicRequest:
        ...
        bt.logging.info(
            f"Processing organic query for source: {synapse.source} (type: {request_type})"
        )
        ...
        try:
            ...
            miner_scores, failed_miners, successful_miners = (
                self._apply_validation_penalties(miner_responses, validation_results)
            )
            ...
            penalized_empty_uids = self._apply_delayed_empty_penalties(
            ...
            if all_penalized_uids:
                penalized_with_hotkeys = [
                    f"{uid}:{self.metagraph.hotkeys[uid]}" for uid in all_penalized_uids
                ]
                bt.logging.info(
                    f"Failed Miners for {synapse.source} request at {request_start_time}: {penalized_with_hotkeys}"
                )
    ...
    def _select_miners(self) -> List[int]:
        ...
        selected_miners_with_hotkeys = [
            f"{uid}:{self.metagraph.hotkeys[uid]}" for uid in selected_miners
        ]
        bt.logging.info(
            f"Selected {len(selected_miners)} miners for query: {selected_miners_with_hotkeys}"
        )
                        ...
                        if data_count > 0:
                            if self._validate_miner_data_format(synapse, data, uid):
                                ...
                                ORGANIC_MINER_RESULTS.labels(
                                    miner_uid=uid,
                                    result_type="failure_format_validation",
                                ).inc()
                        else:
                            miner_responses[uid] = data
                            miner_data_counts[uid] = data_count
    ...
    async def _apply_basic_penalties(
        ...
        for uid in non_responsive_uids:
            bt.logging.info(
                f"Applying penalty to non-responsive miner {uid}:{self.metagraph.hotkeys[uid]}"
            )
            self.evaluator.scorer.apply_ondemand_penalty(uid=uid, mult_factor=1.0)
            ORGANIC_MINER_RESULTS.labels(
                miner_uid=uid, result_type="failure_non_responsive"
            ).inc()
        ...
        if all_empty:
            ...
            if verification_data:
                ...
                for uid in empty_uids:
                    bt.logging.info(
                        f"Applying penalty to miner {uid}:{self.metagraph.hotkeys[uid]} for returning empty results when data exists"
                    )
                    self.evaluator.scorer.apply_ondemand_penalty(
                        uid=uid, mult_factor=1.0
                    )
                    ORGANIC_MINER_RESULTS.labels(
                        miner_uid=uid, result_type="failure_empty_when_data_exists"
                    ).inc()

                # Return verification data as response
                processed_data = [
                    self._create_entity_dictionary(item) for item in verification_data
                ]
    ...
    def _apply_delayed_empty_penalties(
        ...
        if has_valid_pooled_data or has_verification_data:
            ...
            for uid in empty_uids:
                bt.logging.info(
                    f"Applying delayed penalty to empty miner {uid}:{self.metagraph.hotkeys[uid]} - data was available"
                )
                self.evaluator.scorer.apply_ondemand_penalty(uid=uid, mult_factor=1.0)
                ORGANIC_MINER_RESULTS.labels(
                    miner_uid=uid, result_type="failure_empty_when_data_exists"
        ...
        for uid, miner_count in miner_data_counts.items():
            if miner_count > 0:
                ...
                if (
                    ...
                    bt.logging.info(
                        f"Miner {uid}:{self.metagraph.hotkeys[uid]}: fake data padding detected - {miner_count} posts vs {verification_count} verified, applying full penalty"
                    )
                    self.evaluator.scorer.apply_ondemand_penalty(
                        uid=uid, mult_factor=1.0
                    )
                    ORGANIC_MINER_RESULTS.labels(
                        miner_uid=uid, result_type="failure_fake_data_padding"
                    ).inc()
                    continue
                ...
                if mult_factor > 0.1:  # Only penalize underperformance of > 10%
                    bt.logging.info(
                        f"Miner {uid}:{self.metagraph.hotkeys[uid]}: {miner_count} posts vs {verification_count} verified "
                        f"({underperformance_ratio:.1%} underperformance, {mult_factor:.2f} penalty)"
                    )
                    self.evaluator.scorer.apply_ondemand_penalty(
                        uid=uid, mult_factor=mult_factor
                    ...
                    ORGANIC_MINER_RESULTS.labels(
                        miner_uid=uid,
                        result_type="failure_volume_underperformance_verification",
                    ).inc()
    ...
    async def _perform_validation(
        ...
        for uid, posts in miner_responses.items():
            ...
            bt.logging.info(
                f"Validating {num_to_validate} posts from miner {uid}:{self.metagraph.hotkeys[uid]}"
            )
            ...
            for post in selected_posts:
                ...
                task = self._validate_entity(
                    synapse=synapse, entity=post, post_id=post_id, miner_uid=uid
                )
        ...
        if validation_tasks:
            ...
            for i, miner_post_key in enumerate(miner_post_keys):
                ...
                if isinstance(result, Exception):
                    bt.logging.error(
                        f"Validation error for {miner_post_key}: {str(result)}"
                    )
        ...
        bt.logging.info(
            f"Simplified validation completed: {sum(validation_results.values())}/{len(validation_results)} passed"
        )
    ...
    async def _validate_entity(
        self, synapse: OrganicRequest, entity: DataEntity, post_id: str, miner_uid: int
    ) -> bool:
        """
        ...
        try:
            ...
            if synapse.source.upper() == "X" and on_demand_utils.is_nested_format(
                entity
            ):
    ...
    def _validate_request_fields(
        self, synapse: OrganicRequest, entity: DataEntity, miner_uid: int
    ) -> bool:
        """
        ...
        try:
            if synapse.source.upper() == "X":
                return self._validate_x_request_fields(
                    synapse, x_entity=entity, miner_uid=miner_uid
                )
            elif synapse.source.upper() == "REDDIT":
                return self._validate_reddit_request_fields(
                    synapse, reddit_entity=entity, miner_uid=miner_uid
                )
        except Exception as e:
    ...
    def _validate_x_request_fields(
        self, synapse: OrganicRequest, x_entity: DataEntity, miner_uid: int
    ) -> bool:
        ...
        if synapse.url:
            ...
            if utils.normalize_url(post_url) != utils.normalize_url(synapse.url):
                bt.logging.debug(
                    f"Miner {miner_uid}:{hotkey} URL mismatch: {post_url} != {synapse.url}"
                )
                return False
        ...
        if synapse.keywords:
            ...
            if keyword_mode == "all":
                if not all(
                    ...
                    bt.logging.debug(
                        f"Miner {miner_uid}:{hotkey} not all keywords ({synapse.keywords}) found in post: {post_text}"
                    )
                    return False
                ...
                if not any(
                    ...
                    bt.logging.debug(
                        f"Miner {miner_uid}:{hotkey} none of the keywords ({synapse.keywords}) found in post: {post_text}"
                    )
                    return False
    ...
    def _validate_reddit_request_fields(
        self, synapse: OrganicRequest, reddit_entity: DataEntity, miner_uid: int
    ) -> bool:
        ...
        if synapse.keywords:
            ...
            if keyword_mode == "all":
                ...
                keyword_in_content = (
                    any(keyword.lower() in content_text for keyword in synapse.keywords)
                    if content_text
                    else False
                )
