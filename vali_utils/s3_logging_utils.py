...
def log_s3_validation_table(
    ...
    table.add_row(
        "Status", f"[{status_color}]{status_emoji} {status_text}[/{status_color}]"
    )
    ...
    if hasattr(result, "enhanced_validation") and result.enhanced_validation:
        ...
        job_completion_pct = (
            (total_jobs / expected_jobs * 100) if expected_jobs > 0 else 0
        )
        table.add_row(
            "Jobs Coverage",
    ...
    if hasattr(result, "effective_size_bytes") and hasattr(result, "job_coverage_rate"):
        ...
        table.add_row(
            "Job Coverage", f"{coverage:.1f}% (penalty: {coverage_penalty:.2f}x)"
        )
        ...
        if effective_size > 0:
            ...
            table.add_row("Effective Size", f"[red]0 B[/red] (validation failed)")
    ...
    if result.quality_metrics:
        ...
        if job_match > 0:
            job_match_status = format_percentage_with_status(
                job_match, 95.0, inverse=False
            )
            table.add_row("Job Match Rate", job_match_status)

        # Scraper Success (higher is better, threshold 70%)
        if scraper_rate > 0:
            scraper_status = format_percentage_with_status(
                scraper_rate, 70.0, inverse=False
            )
    ...
    if hasattr(result, "enhanced_validation") and result.enhanced_validation:
        ...
        if enhanced.entities_validated > 0:
            passed_color = (
                "green"
                if enhanced.entities_passed_scraper >= enhanced.entities_validated * 0.7
                else "yellow"
            )
            table.add_row(
                "Entities Validated",
        ...
        if enhanced.entities_checked_for_job_match > 0:
            match_color = (
                "green"
                if enhanced.entities_matched_job
                >= enhanced.entities_checked_for_job_match * 0.95
                else "yellow"
            )
            table.add_row(
                "Job Content Match",
        ...
        if (
            hasattr(enhanced, "sample_validation_results")
            and enhanced.sample_validation_results
        ):
            table.add_row("", "")
            table.add_row("[bold]Scraper Details[/bold]", "")

            for i, detail in enumerate(
                enhanced.sample_validation_results
            ):  # Show all validated
        ...
        if (
            hasattr(enhanced, "sample_job_mismatches")
            and enhanced.sample_job_mismatches
        ):
            table.add_row("", "")
            table.add_row("[bold yellow]Job Mismatches[/bold yellow]", "")

            for i, mismatch in enumerate(
                enhanced.sample_job_mismatches[:3]
            ):  # Show max 3
    ...
    if pagination_stats:
        ...
        if "files_per_page" in pagination_stats:
            table.add_row(
                "Avg Files/Page", f"~{pagination_stats['files_per_page']:,} files"
            )

        if "retrieval_time" in pagination_stats:
            table.add_row(
                "Retrieval Time", f"{pagination_stats['retrieval_time']:.1f}s"
            )
...
def log_s3_validation_compact(result: S3ValidationResult, uid: int, hotkey: str):
    """
    ...
    # Get key metrics
    dup_pct = (
        result.quality_metrics.get("duplicate_percentage", 0)
        if result.quality_metrics
        else 0
    )
    job_match = (
        result.quality_metrics.get("job_match_rate", 0) if result.quality_metrics else 0
    )
    scraper_rate = (
        result.quality_metrics.get("scraper_success_rate", 0)
        if result.quality_metrics
        else 0
    )
...
def log_s3_validation_header():
    ...
    console.print(
        "[bold cyan]S3 Validation Results (DuckDB Competition Model)[/bold cyan]"
    )
    console.print("[bold cyan]" + "=" * 100 + "[/bold cyan]")
    console.print()
