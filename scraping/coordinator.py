...
class ScraperCoordinator:
    ...
    def run_in_background_thread(self):
        ...
        logger.info("Starting ScrapingCoordinator in a background thread.")
    ...
    def stop(self):
        logger.info("Stopping the ScrapingCoordinator.")
        self.is_running = False
    ...
    async def _start(self):
        ...
        while self.is_running:
            ...
            if not scraper_ids_to_scrape_now:
                logger.trace("Nothing ready to scrape yet. Trying again in 15s.")
            ...
            for scraper_id in scraper_ids_to_scrape_now:
                ...
                for config in scrape_configs:
                    ...
                    logger.trace(f"Adding scrape task for {scraper_id}: {config}.")
                    self.queue.put_nowait(functools.partial(scraper.scrape, config))

                self.tracker.on_scrape_scheduled(scraper_id, now)

        logger.info("Coordinator shutting down. Waiting for workers to finish.")
    ...
    async def _worker(self, name):
        ...
        while self.is_running:
            try:
                ...
                logger.error("Worker " + name + ": " + traceback.format_exc())
