...
class YouTubeApifyTranscriptScraper(Scraper):
    """
    ...
    2. Apify YouTube Transcript actor for transcript extraction
    ...
    YOUTUBE_TRANSCRIPT_ACTOR_ID = "faVsWy9VTSNVIhWpR"
    ...
    async def _scrape_video_ids(self, video_ids: List[str], date_range: DateRange) -> List[DataEntity]:
        """
        Scrape transcripts for specific video IDs using Apify actor.
        ...
        for video_id in video_ids:
            ...
            try:
                ...
                # Try to get the transcript using Apify
                try:
                    transcript_data = await self._get_transcript_apify(video_id)
            ...
            try:
                ...
                for video_info in channel_videos:
                    try:
                        ...
                        # Get the transcript using Apify
                        transcript_data = await self._get_transcript_apify(video_id)
                        if not transcript_data:
    ...
    async def _get_transcript_apify(self, video_id: str) -> Optional[List[Dict[str, Any]]]:
        """
        Get transcript for a YouTube video using the Apify actor.

        Args:
            video_id: YouTube video ID

        Returns:
            List of transcript segments or None if no transcript available
        """
        try:
            ...
            run_config = RunConfig(
                actor_id=self.YOUTUBE_TRANSCRIPT_ACTOR_ID,
                debug_info=f"Transcript for {video_id} ({video_url})",
                max_data_entities=10000,  # Allow large transcripts
                timeout_secs=self.APIFY_TIMEOUT_SECS
            )

            bt.logging.info(f"Getting transcript for video {video_id} using Apify actor with URL: {video_url}")

            # Run the actor
            dataset = await self.runner.run(run_config, run_input)

            if not dataset:
                bt.logging.warning(f"No transcript data returned for video {video_id} (URL: {video_url})")
                return None

            bt.logging.debug(f"Apify returned {len(dataset)} items for video {video_id}")

            # Process the transcript data - handle the exact format returned by Apify
            transcript_segments = []
            for item in dataset:
                bt.logging.debug(f"Processing Apify item: {item}")

                # The Apify actor wraps transcript data in a 'data' field
                if isinstance(item, dict) and 'data' in item:
                    transcript_data = item['data']
                    bt.logging.debug(f"Found transcript data with {len(transcript_data)} segments")
                ...
                # Also handle direct format (in case the actor response changes)
                    ...
                    try:
                        ...
                        bt.logging.warning(f"Error parsing transcript segment for video {video_id}: {e}, item: {item}")
                        continue

            if not transcript_segments:
                bt.logging.warning(f"No valid transcript segments found for video {video_id}")
                bt.logging.debug(f"Raw dataset from Apify: {dataset}")
                return None

            bt.logging.info(f"Retrieved {len(transcript_segments)} transcript segments for video {video_id}")
            ...
            bt.logging.error(f"Error getting transcript for video {video_id} (URL: {video_url}): {str(e)}")
            bt.logging.error(traceback.format_exc())
            return None
        ...
        for entity in entities:
            ...
            while attempt < self.MAX_VALIDATION_ATTEMPTS:
                try:
                    ...
                    # Verify transcript using Apify
                    try:
                        transcript_data = await self._get_transcript_apify(content_to_validate.video_id)
                        ...
                        if not content_to_validate.transcript:
                            ...
                            results.append(
                                ValidationResult(
                                    is_valid=False,
                                    reason="Transcript no longer available or validation error",
                                    content_size_bytes_validated=entity.content_size_bytes
                                )
                            )
                        break
    ...
    def _verify_transcript(self, api_transcript: List[Dict[str, Any]], stored_transcript: List[Dict[str, Any]]) -> bool:
        ...
        Args:
            api_transcript: The transcript from the Apify actor.
            stored_transcript: The transcript to validate.

        Returns:
            True if the transcripts match sufficiently, False otherwise.
        """
        if not api_transcript and not stored_transcript:
            return True

        if not api_transcript or not stored_transcript:
            return False
        ...
        while len(videos) < max_results and api_calls < max_api_calls:
            ...
            try:
                ...
                for item in playlist_response.get("items", []):
                    ...
                    # Check for transcript availability using Apify
                    try:
                        transcript = await self._get_transcript_apify(video_id)
                        if transcript:
                            videos.append({
                                "id": video_id,
                        ...
                        bt.logging.debug(f"No transcript for video {video_id}, skipping: {str(e)}")
                        continue
    ...
    async def _get_channel_videos_httpx(
        ...
        while len(collected) < max_results and api_calls < MAX_API_CALLS:
            ...
            for item in pl_resp["items"]:
                ...
                # Check for transcript using Apify
                transcript = await self._get_transcript_apify(video_id)
                if not transcript:
                    continue
...
async def test_scrape_video():
    ...
    scraper = YouTubeApifyTranscriptScraper()
...
async def main():
    print("\nEnhanced YouTube Apify Transcript Scraper - Test Menu")
    print("=" * 50)
...
if __name__ == "__main__":
    bt.logging.info("Starting YouTube Apify scraper tests...")
    asyncio.run(main())