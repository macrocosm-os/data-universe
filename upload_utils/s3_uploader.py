...
def load_dynamic_lookup() -> Dict[str, List[Dict]]:
    ...
    try:
        ...
        bt.logging.warning(
            "dynamic_desirability/total.json not found, returning empty lookup"
        )
        return {}
...
class S3PartitionedUploader:
    ...
    def _update_processed_state(
        self, job_id: str, new_offset: int, records_processed: int
    ):
        ...
        self.processed_state[job_id].update(
            {
                ...
                "total_records_processed": self.processed_state[job_id].get(
    ...
    def _load_dd_list(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            # Handle old format for backward compatibility
                ...
                bt.logging.warning(
                    "Old DD format detected, consider updating to new job-based format"
                )
    ...
    def _get_label_data(self, source: int, label: str, offset: int = 0) -> pd.DataFrame:
        ...
        try:
            ...
            bt.logging.debug(
                f"Found {len(df)} records for label '{label}' in source {source} (offset: {offset})"
            )
            return df
    ...
    def _get_keyword_data_chunk(
        self, source: int, keyword: str, offset: int = 0
    ) -> pd.DataFrame:
        ...
        try:
            ...
            bt.logging.debug(
                f"Found {len(df)} records for keyword '{keyword}' in source {source} (offset: {offset})"
            )
            return df
    ...
    def _get_label_and_keyword_data(
        self, source: int, label: str, keyword: str, offset: int = 0
    ) -> pd.DataFrame:
        ...
        try:
            ...
            bt.logging.debug(
                f"Found {len(df)} records for label '{label}' AND keyword '{keyword}' in source {source} (offset: {offset})"
            )
            return df

        except Exception as e:
            bt.logging.error(
                f"Error querying label+keyword data for {source}/{label}/{keyword}: {e}"
            )
            return pd.DataFrame()
                ...
                result_df = pd.DataFrame(
                    {
                        ...
                        "username": df["decoded_content"].apply(
                            lambda x: x.get("username")
                        ),
                        "communityName": df["decoded_content"].apply(
                            lambda x: x.get("communityName")
                        ),
                        "body": df["decoded_content"].apply(lambda x: x.get("body")),
                        "title": df["decoded_content"].apply(lambda x: x.get("title")),
                        "createdAt": df["decoded_content"].apply(
                            lambda x: x.get("createdAt")
                        ),
                        "dataType": df["decoded_content"].apply(
                            lambda x: x.get("dataType")
                        ),
                        "parentId": df["decoded_content"].apply(
                        ...
                        "is_nsfw": df["decoded_content"].apply(
                            lambda x: x.get("is_nsfw")
                        ),
                        "score": df["decoded_content"].apply(lambda x: x.get("score")),
                        "upvote_ratio": df["decoded_content"].apply(
                            lambda x: x.get("upvote_ratio")
                        ),
                        "num_comments": df["decoded_content"].apply(
                            lambda x: x.get("num_comments")
                        ),
                    }
                )
            else:
                ...
                result_df = pd.DataFrame(
                    {
                        ...
                        "tweet_hashtags": df["decoded_content"].apply(
                            lambda x: x.get("tweet_hashtags", [])
                        ),
                        "timestamp": df["decoded_content"].apply(
                            lambda x: x.get("timestamp")
                        ),
                        "url": df["decoded_content"].apply(lambda x: x.get("url")),
                        "media": df["decoded_content"].apply(lambda x: x.get("media")),
                        "user_id": df["decoded_content"].apply(
                            lambda x: x.get("user_id")
                        ),
                        "user_display_name": df["decoded_content"].apply(
                            lambda x: x.get("user_display_name")
                        ),
                        "user_verified": df["decoded_content"].apply(
                            lambda x: x.get("user_verified")
                        ),
                        "tweet_id": df["decoded_content"].apply(
                            lambda x: x.get("tweet_id")
                        ),
                        "is_reply": df["decoded_content"].apply(
                            lambda x: x.get("is_reply")
                        ),
                        "is_quote": df["decoded_content"].apply(
                            lambda x: x.get("is_quote")
                        ),
                        "conversation_id": df["decoded_content"].apply(
                            lambda x: x.get("conversation_id")
                        ),
                        "in_reply_to_user_id": df["decoded_content"].apply(
                            lambda x: x.get("in_reply_to_user_id")
                        ),
                        "language": df["decoded_content"].apply(
                            lambda x: x.get("language")
                        ),
                        "in_reply_to_username": df["decoded_content"].apply(
                            lambda x: x.get("in_reply_to_username")
                        ),
                        "quoted_tweet_id": df["decoded_content"].apply(
                            lambda x: x.get("quoted_tweet_id")
                        ),
                        "like_count": df["decoded_content"].apply(
                            lambda x: x.get("like_count")
                        ),
                        "retweet_count": df["decoded_content"].apply(
                            lambda x: x.get("retweet_count")
                        ),
                        "reply_count": df["decoded_content"].apply(
                            lambda x: x.get("reply_count")
                        ),
                        "quote_count": df["decoded_content"].apply(
                            lambda x: x.get("quote_count")
                        ),
                        "view_count": df["decoded_content"].apply(
                            lambda x: x.get("view_count")
                        ),
                        "bookmark_count": df["decoded_content"].apply(
                            lambda x: x.get("bookmark_count")
                        ),
                        "user_blue_verified": df["decoded_content"].apply(
                            lambda x: x.get("user_blue_verified")
                        ),
                        "user_description": df["decoded_content"].apply(
                            lambda x: x.get("user_description")
                        ),
                        "user_location": df["decoded_content"].apply(
                            lambda x: x.get("user_location")
                        ),
                        "profile_image_url": df["decoded_content"].apply(
                            lambda x: x.get("profile_image_url")
                        ),
                        "cover_picture_url": df["decoded_content"].apply(
                            lambda x: x.get("cover_picture_url")
                        ),
                        "user_followers_count": df["decoded_content"].apply(
                            lambda x: x.get("user_followers_count")
                        ),
                        "user_following_count": df["decoded_content"].apply(
                            lambda x: x.get("user_following_count")
                        ),
                    }
                )
    ...
    def _upload_data_chunk(
        self, df: pd.DataFrame, source: int, job_id: str, s3_creds: Dict
    ) -> bool:
        ...
        try:
            ...
            # Upload to S3
            upload_success = self.s3_auth.upload_file_with_path(
                local_path, s3_path, s3_creds
            )
            ...
            if upload_success:
                ...
                bt.logging.error(
                    f"Failed to upload {len(raw_df)} records to job {job_id}"
                )
    ...
    def _process_job(self, job_id: str, job_config: Dict, s3_creds: Dict) -> bool:
        ...
        bt.logging.info(
            f"Processing job {job_id} ({log_msg}), starting from offset: {offset}"
        )
        ...
        while True:
            ...
            if search_type == "label":
                ...
                chunk_df = self._get_label_and_keyword_data(
                    source, label, keyword, offset
                )
            else:
            ...
            if chunk_df.empty:
                bt.logging.info(
                    f"No new data for job {job_id}, total processed this run: {total_processed}"
                )
                break
