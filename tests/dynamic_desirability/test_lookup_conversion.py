...
"""Testing to_lookup that converts a json desirability index to a DynamicDesirabilityLookup object."""

def formatted_print(distribution):
    """Converts a DynamicDesirabilityLookup distribution to JSON-like format."""
    json_data = []
...
bt.logging.info("Converting to DynamicDesirabilityLookup...")
lookup = to_lookup(file_path)
primitive_lookup = DynamicDesirabilityLookup.to_primitive_dynamic_desirability_lookup(lookup)
