...
class Job(StrictBaseModel):
    ...
    job_weight: float = Field(
        ge=-1, le=5, description="The reward for data associated with the job."
    )
...
class JobMatcher(StrictBaseModel):
    ...
    def find_matching_jobs(
        self, data_keyword: str, data_label: str, data_timebucket: int
    ) -> List[Job]:
...
class PrimitiveDataSourceDesirability:
    ...
    def __init__(
        ...
        for job in self.jobs:
            ...
            if (
                "start_timebucket" in job
                and job["start_timebucket"]
                and "start_timebucket" not in job
            ):
                start_dt = datetime.fromisoformat(job["start_timebucket"])
                job["start_timebucket"] = utils.time_bucket_id_from_datetime(start_dt)

            if (
                "end_timebucket" in job
                and job["end_timebucket"]
                and "end_timebucket" not in job
            ):
...
class PrimitiveDataDesirabilityLookup:
    ...
    def find_matching_jobs(
        self, data_source: DataSource, keyword: str, label: str, data_timebucket: int
    ) -> List[Dict]:
