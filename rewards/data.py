...
class Job(StrictBaseModel):
    ...
    keyword: str | None = Field(description="Optionally, the keyword associated with the job.")

    label: str | None = Field(description="The actual incentivized label or keyword.")

    job_weight: float = Field(ge=-1, le=5, description="The reward for data associated with the job.")

    # Store as time bucket IDs instead of datetime strings
    start_timebucket: int | None = Field(
        default=None, description="Optionally, the earliest viable time bucket at which data is accepted."
    )

    end_timebucket: int | None = Field(
        default=None, description="Optionally, the latest viable time bucket at which data is accepted."
    )
...
class JobMatcher(StrictBaseModel):
    ...
    def find_matching_jobs(self, data_keyword: str, data_label: str, data_timebucket: int) -> list[Job]:
...
class DataSourceDesirability(StrictBaseModel):
    ...
    def model_dump_json(self, **kwargs):
        ...
        result = {"weight": self.weight, "default_scale_factor": self.default_scale_factor, "jobs": jobs_data}
...
class DataDesirabilityLookup(StrictBaseModel):
    ...
    def __str__(self) -> str:
        ...
        return json.dumps({"distribution": distribution_dict, "max_age_in_hours": self.max_age_in_hours}, indent=4)
    ...
    def validate_distribution(
        ...
        if sum(data_source_desirability.weight for data_source_desirability in distribution.values()) != 1.0:
...
class PrimitiveDataDesirabilityLookup:
    ...
    def __init__(self, distribution: dict[DataSource, PrimitiveDataSourceDesirability], max_age_in_hours: int):
        self.distribution = distribution
        self.max_age_in_hours = max_age_in_hours

    def find_matching_jobs(self, data_source: DataSource, keyword: str, label: str, data_timebucket: int) -> list[dict]:
        ...
        return self.distribution[data_source].find_matching_jobs(keyword, label, data_timebucket)
