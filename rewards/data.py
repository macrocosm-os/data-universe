...
class DataSourceDesirability(StrictBaseModel):
    ...
    def validate_label_scale_factors(
        ...
        for label, scale_factor in value.items():
            # Max label weight for one active validator putting 100% on one label = 35 + 1 from default.json if applicable
            if scale_factor < -1.0 or scale_factor > 36:
                raise ValueError(
                    f"Label {label} scale factors must be between -1 and 36, inclusive."
                )
        return value
